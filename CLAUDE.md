...
**階段 1: Chat 模式（95%完成）**
- ChatGPT 風格對話介面（左側對話記錄，右側推薦）
- 智能商品推薦（LLM 驅動）
- 商品多選功能（Checkbox）
- 前往穿搭工作室按鈕
- 檔案：`/frontend/app/chat/page.tsx`

**階段 2: Outfit 模式（90%完成）**
- AI 自動生成 6 套 LOOK（OpenAI GPT-4o Mini）
- 支援洋裝（1件）與上下身組合（2件）
- LOOK 卡片展示（2行×3列）
- React DnD 拖拽交換功能
- 價格計算與顯示
- API：`/api/outfit/generate`
- Store：`/store/outfitStore.ts`

**階段 3: Try-On 模式（70%完成）**
- 多 LOOK 複選功能（批次試穿）
- PhotoUpload 組件整合
- 批次試穿 API 調用
- 試穿結果展示（卡片式佈局）
- 下載試穿圖與加入購物車按鈕
- 圖片 URL 轉換修復（相對→絕對路徑）
- 兩步試穿法（上衣→下身，準確度待優化⚠️）
- API：`/api/tryon`
- 後端：Gemini 2.5 Flash (主要) + HF Space (備援)
...
### 🚧 開發中功能
- **虛擬試穿優化** - 兩步試穿準確度改進（⚠️ 已知問題）
- **購物車頁面 UI** - 列表展示、數量調整、刪除功能
- **結帳流程** - 收件資訊、付款方式、訂單確認

### ❌ 待修復問題
- **虛擬試穿準確度** - AI 對 `only top/bottom` 指令理解有限，下身替換效果不佳
- **中文輸入編碼問題** - curl 發送中文時出現亂碼 (次要問題)
  - 英文查詢天氣功能完全正常
  - 中文查詢在 curl 測試時出現編碼問題，實際前端應該正常
  - 建議在實際前端介面測試中文天氣查詢

### 📋 TODO 清單（優先級排序）- 2026-01-03 更新（白板設計）

#### 🔴 P0：Django 後端建設（Phase 1）
1. 建立 Django 專案骨架
2. Product Model（雙圖片欄位：image + image_nobg）
3. rembg 去背服務整合
4. 批量去背 79 張圖片
5. REST API 端點實作
